...
describe("multi-step integration scenarios", () => {
	...
	test("custom configuration is never replaced by automatic repair", () => {
		setEnv("https://proxy.example.com", "sk-abc123");
		const custom = "providers:\n  custom:\n    baseUrl: https://custom.example.test\n    apiKey: LITELLM_API_KEY\n";
		fs.writeFileSync(modelsPath, custom);
		const result = autoFixModelsConfig(modelsPath);
		expect(result.fixed).toBe(false);
		expect(result.changes[0]).toContain("Skipped custom models.yml");
		expect(fs.readFileSync(modelsPath, "utf8")).toBe(custom);
