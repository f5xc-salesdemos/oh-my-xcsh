...
pub trait SlavePty {
    ...
    /// Spawns a command after registering a callback that runs in the child immediately after fork.
    ///
    /// The callback is Unix-only because it has the same safety contract as
    /// [`std::os::unix::process::CommandExt::pre_exec`]. It is registered before portable-pty's
    /// terminal setup callback so callers can establish process-wide security state before any other
    /// child hook runs.
    #[cfg(unix)]
    fn spawn_command_with_pre_exec(
        &self,
        _cmd: CommandBuilder,
        _pre_exec: Box<dyn FnMut() -> IoResult<()> + Send + Sync + 'static>,
    ) -> Result<Box<dyn Child + Send + Sync>, Error> {
        anyhow::bail!("this PTY backend does not support child pre-exec callbacks")
    }
}
