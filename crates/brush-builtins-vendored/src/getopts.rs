...
impl builtins::Command for GetOptsCommand {
	...
	/// information TODO: we can safely remove this after the issue is resolved
	...
	fn execute(
		...
		futures::future::lazy(move |_| {
			let Some((args, treat_unknown_options_as_failure)) = self.option_spec() else {
			...
			self.update_option_variables(&mut context, variable_value, new_optarg, new_optind)?;
...
impl GetOptsCommand {
	fn update_option_variables(
		&self,
		context: &mut brush_core::ExecutionContext<'_>,
		variable_value: String,
		new_optarg: Option<String>,
		new_optind: usize,
...
impl GetOptsCommand {
	fn option_spec(&self) -> Option<(HashMap<char, bool>, bool)> {
		...
		Some((args, treat_unknown_options_as_failure))
	}
}
