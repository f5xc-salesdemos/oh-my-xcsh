...
fn split_line_by_ifs(ifs: &str, line: &str, max_fields: Option<usize>) -> VecDeque<String> {
	...
	// First, trim from the prefix and suffix of the string any matching chars that
	// are *both* whitespace and present in the IFS string.
	...
	// Now, iterate through the string, manually splitting it by the shell's rules
	// for honoring IFS. We implement this by hand because we need to ensure that
	// any IFS character is a valid delimiter, *but* consecutive adjacent IFS
	// characters are considered a single delimiter if and only if they are one of
	// the 3 default whitespace characters (i.e., ' ', '\t', or '\n').
