	...
	fn apply_ansi_u16(&mut self, seq: &[u16]) {
		if is_sgr_u16(seq) {
			...
			if uri.is_empty() {
				...
				self.hyperlink = Some(seq.to_vec());
			}
		}
	}
...
fn wrap_single_line(line: &[u16], width: usize, tab_width: usize) -> SmallVec<[Vec<u16>; 4]> {
	...
	if visible_width_u16(line, tab_width) <= width {
		...
		if state.hyperlink.is_some() {
			only.extend_from_slice(&OSC8_CLOSE);
		}
	...
	if !current_line.is_empty() {
		if state.hyperlink.is_some() {
			current_line.extend_from_slice(&OSC8_CLOSE);
		}
		wrapped.push(current_line);
	}
...
mod tests {
	...
	#[test]
	fn test_wrap_text_with_ansi_isolates_osc8_on_every_physical_line() {
		let data = to_u16("\x1b]8;;https://example.test/very-long\x07abcdefghij\x1b]8;;\x07 tail");
		let lines = wrap_text_with_ansi_impl(&data, 4, DEFAULT_TAB_WIDTH);
		assert!(lines.len() >= 3);
		for line in lines.iter().take(3) {
			let text = String::from_utf16_lossy(line);
			let opens = text
				.matches("\x1b]8;;https://example.test/very-long\x07")
				.count();
			let closes = text.matches("\x1b]8;;\x07").count();
			assert_eq!(opens, 1, "missing hyperlink reopen: {text:?}");
			assert_eq!(closes, 1, "unbalanced hyperlink line: {text:?}");
		}
		let tail = String::from_utf16_lossy(lines.last().unwrap());
		assert!(!tail.contains("https://example.test"), "link leaked into tail: {tail:?}");

		let punctuated = to_u16(
			"prefix \x1b]8;;https://example.com/very/long/path/that/wraps\x07https://example.com/very/long/path/that/wraps\x1b]8;;\x07 suffix",
		);
		for line in wrap_text_with_ansi_impl(&punctuated, 20, DEFAULT_TAB_WIDTH) {
			let text = String::from_utf16_lossy(&line);
			let opens = text
				.matches("\x1b]8;;https://example.com/very/long/path/that/wraps\x07")
				.count();
			let closes = text.matches("\x1b]8;;\x07").count();
			assert_eq!(opens, closes, "unbalanced punctuated hyperlink line: {text:?}");
		}
	}
}
